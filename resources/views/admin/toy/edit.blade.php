                ...
                <div class="mb-3">
                    ...
                    <div style="max-width: 350px; max-height: 350px">
                        <img class="w-100 h-100" style="display:block;"
                            src="{{ $toy->image ? asset('img/' . $toy->image) : 'https://placehold.co/300/orange/white?text=Toy' }}"
            ...
            <div class="col-8 d-flex justify-content-center  flex-column ps-5">
                    ...
                    <select class="form-select" aria-label="Default select example" name="category">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ $category->name == $toy->category->name ? 'selected' : '' }}>{{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-floating mb-3">
                    <input type="number" class="form-control" id="floatingInput" name="price"
                        placeholder="name@example.com" value="{{ $toy->price }}">
